...
//+------------------------------------------------------------------+
int OnInit() {
   Print("=== SMC ICT Scalping EA v4.0 (MT5) Initialized ===");
   Print("Account Balance: ", account.Balance(), " ", account.Currency());
...
void OnDeinit(const int reason) {
   Print("=== SMC ICT Scalping EA v4.0 (MT5) Deinitialized ===");
   Print("Reason: ", reason);
   Comment("");
}
...
void OnTick() {
   ...
   if(dt.day_of_year != TimeDayOfYear(lastTradeDate)) {
      ResetDailyStatistics();
   }
...
double CalculateLotSize(ENUM_ORDER_TYPE orderType) {
   ...
   if(tickValue > 0 && stopLossPoints > 0 && tickSize > 0) {
      double maxLotByRisk = riskAmount / (stopLossPoints * tickValue / tickSize);
...
void ManageActiveTrades() {
   for(int i = ArraySize(activeTrades) - 1; i >= 0; i--) {
      ...
      double currentPrice = (activeTrades[i].type == POSITION_TYPE_BUY) ? 
...
void ClosePartialPosition(int tradeIndex, double percentage, string tpLevel) {
   if(!position.SelectByTicket(activeTrades[tradeIndex].ticket)) return;
   ...
   bool closed = trade.PositionClosePartial(activeTrades[tradeIndex].ticket, lotsToClose);
   
   if(closed) {
      activeTrades[tradeIndex].remainingLots -= lotsToClose;
      Print(tpLevel, " reached! Closed ", lotsToClose, " lots");
...
void UpdateDailyProfitLoss() {
   ...
   HistorySelect(iTime(TradingSymbol, PERIOD_D1, 0), today);
   ...
   for(int i = HistoryDealsTotal() - 1; i >= 0; i--) {
      ...
      MqlDateTime dtDeal, dtToday;
      TimeToStruct(dealTime, dtDeal);
      TimeToStruct(today, dtToday);
      
      if(dtDeal.day_of_year == dtToday.day_of_year) {
...
int AnalyzeMarketEntry() {
   ...
   if(EnableOrderBlocks) {
      ...
      if(obSignal == ORDER_TYPE_BUY) {
         bullishSignal = true;
         confluencePoints++;
      }
      else if(obSignal == ORDER_TYPE_SELL) {
         bearishSignal = true;
         confluencePoints++;
      }
   }
   ...
   if(EnableFVG) {
      ...
      if(fvgSignal == ORDER_TYPE_BUY) {
         bullishSignal = true;
         confluencePoints++;
      }
      else if(fvgSignal == ORDER_TYPE_SELL) {
         bearishSignal = true;
         confluencePoints++;
      }
   }
   ...
   if(EnableBOS) {
      ...
      if(bosSignal == ORDER_TYPE_BUY) {
         bullishSignal = true;
         confluencePoints++;
      }
      else if(bosSignal == ORDER_TYPE_SELL) {
         bearishSignal = true;
         confluencePoints++;
      }
   }
   ...
   if(EnableCHoCH) {
      ...
      if(chochSignal == ORDER_TYPE_BUY) {
         bullishSignal = true;
         confluencePoints++;
      }
      else if(chochSignal == ORDER_TYPE_SELL) {
         bearishSignal = true;
         confluencePoints++;
      }
   }
   ...
   if(EnableLiquiditySweep) {
      ...
      if(lsSignal == ORDER_TYPE_BUY) {
         bullishSignal = true;
         confluencePoints++;
      }
      else if(lsSignal == ORDER_TYPE_SELL) {
         bearishSignal = true;
         confluencePoints++;
      }
   }
...
void OpenBuyTrade(double lotSize) {
   ...
   double point = SymbolInfoDouble(TradingSymbol, SYMBOL_POINT);
   double stopLoss = ask - (StopLossPips * 10 * point);
   
   if(trade.Buy(lotSize, TradingSymbol, ask, stopLoss, 0, "SMC ICT EA v4.0")) {
      ulong ticket = trade.ResultOrder();
      Print("BUY order opened: Ticket=", ticket, " Lot=", lotSize, " Price=", ask, " SL=", stopLoss);
      ...
      activeTrades[size].entryPrice = ask;
      activeTrades[size].lotSize = lotSize;
      activeTrades[size].remainingLots = lotSize;
      activeTrades[size].type = POSITION_TYPE_BUY;
...
void OpenSellTrade(double lotSize) {
   ...
   double point = SymbolInfoDouble(TradingSymbol, SYMBOL_POINT);
   double stopLoss = bid + (StopLossPips * 10 * point);
   
   if(trade.Sell(lotSize, TradingSymbol, bid, stopLoss, 0, "SMC ICT EA v4.0")) {
      ulong ticket = trade.ResultOrder();
      Print("SELL order opened: Ticket=", ticket, " Lot=", lotSize, " Price=", bid, " SL=", stopLoss);
      ...
      activeTrades[size].ticket = ticket;
      activeTrades[size].entryPrice = bid;
      activeTrades[size].lotSize = lotSize;
      activeTrades[size].remainingLots = lotSize;
      activeTrades[size].type = POSITION_TYPE_SELL;
      activeTrades[size].tp1Closed = false;
      activeTrades[size].tp2Closed = false;
      activeTrades[size].tp3Closed = false;
      activeTrades[size].tp4Closed = false;
...
int CountOpenPositions() {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      if(position.SelectByIndex(i)) {
         if(position.Magic() == MagicNumber && position.Symbol() == TradingSymbol) {
            count++;
         }
      }
   }
...
string GetStatusComment() {
   string comment = "";
   comment += "=== SMC ICT Scalping EA v4.0 (MT5) ===\n";
   comment += "Symbol: " + TradingSymbol + " | Timeframe: M1\n";
   comment += "Account: " + DoubleToString(account.Balance(), 2) + " " + account.Currency() + "\n";
   comment += "-----------------------------------\n";
